...
connectiom = cx_Oracle.connect(user="vova", password="vova")

cursor = connectiom.cursor()

print("connected")
print("Получаем средню длину лепестка. Среднее значение равно:" )
query1 = cursor.execute("""select avg(vovik.Лист_длин) from vovik
where Номер_вида =1
union
...
py.dashboard_ops.upload(my_dboard, 'Lab3_Dashboard_done3')